package food.order.restaurant.infrastructure.kafka.publisher;
...
import food.order.restaurant.application.config.RestaurantServiceConfigData;
import food.order.restaurant.application.ports.output.message.OrderRejectedMessagePublisher;
import food.order.restaurant.domain.event.OrderRejectedEvent;
import food.order.restaurant.infrastructure.kafka.mapper.RestaurantMessagingDataMapper;
...
public class OrderRejectedKafkaMessagePublisher implements OrderRejectedMessagePublisher {
    ...
    public void publish(OrderRejectedEvent orderRejectedEvent) {
        ...
        try {
            RestaurantApprovalResponseAvroModel restaurantApprovalResponseAvroModel = restaurantMessagingDataMapper.orderRejectedEventToRestaurantApprovalResponseAvroModel(orderRejectedEvent);

            kafkaProducer.send(restaurantServiceConfigData.getRestaurantApprovalResponseTopicName(), orderId, restaurantApprovalResponseAvroModel, kafkaMessageHelper.getKafkaCallback(restaurantServiceConfigData.getRestaurantApprovalResponseTopicName(), restaurantApprovalResponseAvroModel, orderId, "RestaurantApprovalResponseAvroModel"));
